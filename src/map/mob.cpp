		...
		if (mob->get_bosstype() == BOSSTYPE_MVP) { // MVP  
			...
			if (is_renewal_monster) {    
				...
				if (battle_config.mvp_hp_rate2 != 100)    
					maxhp = maxhp * (double)battle_config.mvp_hp_rate2 / 100.;  
				  
				// Base stats scaling  
				if (battle_config.mvp_str_rate2 != 100)  
					mob->status.str = cap_value(mob->status.str * battle_config.mvp_str_rate2 / 100, 1, USHRT_MAX);  
				if (battle_config.mvp_agi_rate2 != 100)  
					mob->status.agi = cap_value(mob->status.agi * battle_config.mvp_agi_rate2 / 100, 1, USHRT_MAX);  
				if (battle_config.mvp_vit_rate2 != 100)  
					mob->status.vit = cap_value(mob->status.vit * battle_config.mvp_vit_rate2 / 100, 1, USHRT_MAX);  
				if (battle_config.mvp_int_rate2 != 100)  
					mob->status.int_ = cap_value(mob->status.int_ * battle_config.mvp_int_rate2 / 100, 1, USHRT_MAX);  
				if (battle_config.mvp_dex_rate2 != 100)  
					mob->status.dex = cap_value(mob->status.dex * battle_config.mvp_dex_rate2 / 100, 1, USHRT_MAX);  
				if (battle_config.mvp_luk_rate2 != 100)  
					mob->status.luk = cap_value(mob->status.luk * battle_config.mvp_luk_rate2 / 100, 1, USHRT_MAX);  
				  
				// Combat stats scaling  
				if (battle_config.mvp_atk_rate2 != 100)  
					mob->status.rhw.atk = cap_value(mob->status.rhw.atk * battle_config.mvp_atk_rate2 / 100, 1, USHRT_MAX);  
				if (battle_config.mvp_matk_rate2 != 100)  
					mob->status.rhw.atk2 = cap_value(mob->status.rhw.atk2 * battle_config.mvp_atk_rate2 / 100, 1, USHRT_MAX);  
				if (battle_config.mvp_def_rate2 != 100)  
					mob->status.def = cap_value(mob->status.def * battle_config.mvp_def_rate2 / 100, 0, SHRT_MAX);  
				if (battle_config.mvp_mdef_rate2 != 100)  
					mob->status.mdef = cap_value(mob->status.mdef * battle_config.mvp_mdef_rate2 / 100, 0, SHRT_MAX);  
				if (battle_config.mvp_hit_rate2 != 100)  
					mob->status.hit = cap_value(mob->status.hit * battle_config.mvp_hit_rate2 / 100, 1, SHRT_MAX);  
				if (battle_config.mvp_flee_rate2 != 100)  
					mob->status.flee = cap_value(mob->status.flee * battle_config.mvp_flee_rate2 / 100, 1, SHRT_MAX);  
				if (battle_config.mvp_crit_rate2 != 100)  
					mob->status.cri = cap_value(mob->status.cri * battle_config.mvp_crit_rate2 / 100, 1, SHRT_MAX);  
				  
				// Speed scaling (lower is faster)  
				if (battle_config.mvp_aspd_rate2 != 100) {  
					mob->status.amotion = cap_value(mob->status.amotion * 100 / battle_config.mvp_aspd_rate2, 100, USHRT_MAX);  
			...
			if (is_renewal_monster) {    
				...
				if (battle_config.monster_hp_rate2 != 100)    
					maxhp = maxhp * (double)battle_config.monster_hp_rate2 / 100.;  
				  
				// Base stats scaling  
				if (battle_config.monster_str_rate2 != 100)  
					mob->status.str = cap_value(mob->status.str * battle_config.monster_str_rate2 / 100, 1, USHRT_MAX);  
				if (battle_config.monster_agi_rate2 != 100)  
					mob->status.agi = cap_value(mob->status.agi * battle_config.monster_agi_rate2 / 100, 1, USHRT_MAX);  
				if (battle_config.monster_vit_rate2 != 100)  
					mob->status.vit = cap_value(mob->status.vit * battle_config.monster_vit_rate2 / 100, 1, USHRT_MAX);  
				if (battle_config.monster_int_rate2 != 100)  
					mob->status.int_ = cap_value(mob->status.int_ * battle_config.monster_int_rate2 / 100, 1, USHRT_MAX);  
				if (battle_config.monster_dex_rate2 != 100)  
					mob->status.dex = cap_value(mob->status.dex * battle_config.monster_dex_rate2 / 100, 1, USHRT_MAX);  
				if (battle_config.monster_luk_rate2 != 100)  
					mob->status.luk = cap_value(mob->status.luk * battle_config.monster_luk_rate2 / 100, 1, USHRT_MAX);  
				  
				// Combat stats scaling  
				if (battle_config.monster_atk_rate2 != 100)  
					mob->status.rhw.atk = cap_value(mob->status.rhw.atk * battle_config.monster_atk_rate2 / 100, 1, USHRT_MAX);  
				if (battle_config.monster_matk_rate2 != 100)  
					mob->status.rhw.atk2 = cap_value(mob->status.rhw.atk2 * battle_config.monster_atk_rate2 / 100, 1, USHRT_MAX);  
				if (battle_config.monster_def_rate2 != 100)  
					mob->status.def = cap_value(mob->status.def * battle_config.monster_def_rate2 / 100, 0, SHRT_MAX);  
				if (battle_config.monster_mdef_rate2 != 100)  
					mob->status.mdef = cap_value(mob->status.mdef * battle_config.monster_mdef_rate2 / 100, 0, SHRT_MAX);  
				if (battle_config.monster_hit_rate2 != 100)  
					mob->status.hit = cap_value(mob->status.hit * battle_config.monster_hit_rate2 / 100, 1, SHRT_MAX);  
				if (battle_config.monster_flee_rate2 != 100)  
					mob->status.flee = cap_value(mob->status.flee * battle_config.monster_flee_rate2 / 100, 1, SHRT_MAX);  
				if (battle_config.monster_crit_rate2 != 100)  
					mob->status.cri = cap_value(mob->status.cri * battle_config.monster_crit_rate2 / 100, 1, SHRT_MAX);  
				  
				// Speed scaling (lower is faster)  
				if (battle_config.monster_aspd_rate2 != 100) {  
					mob->status.amotion = cap_value(mob->status.amotion * 100 / battle_config.monster_aspd_rate2, 100, USHRT_MAX);  
